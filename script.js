...
// Récupère une blague française via l'API
async function generateFrenchJoke() {
    try {
        const config = {
            headers: {
                'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoiOTA3MTYxNTQ2NjQyMzA1MDQ2IiwibGltaXQiOjEwMCwia2V5IjoiMEF0WWpaY1drT1dabGdocm5MakRsNXJEbGNtQlJlektvalJ5dVJwYzZXdms3UkVZMHciLCJjcmVhdGVkX2F0IjoiMjAyNS0wNS0yNVQxNjozODoxMCswMDowMCIsImlhdCI6MTc0ODE5MTA5MH0.gefP8dPKZ360QUwOwdM2YQHwrqYdRA_oqgpEkcbie2Y',
                'Accept': 'application/json'
            }
        };

        const res = await fetch('https://www.blagues-api.fr/api/random', config);
        ...
        if (data.type === 'single') {
            ...
            jokeEl.innerHTML = `${data.joke}<br><br>${data.answer}`;
        }
    } catch (error) {
        console.error('Erreur lors de la récupération de la blague:', error);
        // En cas d'erreur, on utilise une blague de la liste locale
